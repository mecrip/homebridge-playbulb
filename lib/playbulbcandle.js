...
function PlaybulbCandle(log, name, address, platform) {
    ...
    if(platform.config.effectstemplate !== undefined){
        this.effects_template = platform.config.effectstemplate;
    }
    
    this.placeholder = DEFAULT_PLACEHOLDER;
    if(platform.config.placeholder !== undefined){
        this.placeholder = platform.config.placeholder;
    }
    
    this.effects_handle = DEFAULT_EFFECTS_HANDLE;
    if(platform.config.effectshandle !== undefined){
        this.effects_handle = parseInt(""+platform.config.effectshandle);
    }
    
    var hex = DEFAULT_COLOR;
    if(platform.config.color !== undefined){
        hex = platform.config.color;
    }
...
PlaybulbCandle.prototype.setEffectColor = function(rgb, callback) {
    var effect = this.effects_template.replace(this.placeholder, rgb);
