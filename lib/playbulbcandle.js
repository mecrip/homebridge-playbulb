...
function PlaybulbCandle(log, name, address, platform, accessory) {
    ...
    this.connect(this.homebridgeAcc);
}

PlaybulbCandle.prototype.connect = function (homebridgeAcc) {
    ...
    this.LampService = homebridgeAcc.getService(Service.Lightbulb, "Light");
    ...
    this.BatteryService = homebridgeAcc.getService(Service.BatteryService, "Battery");
    ...
    this.SwitchService=homebridgeAcc.getService(Service.Switch,"Rainbow");
...
PlaybulbCandle.prototype.readHandle = function(handle,callback)
{
    ...
    var gatt_cmd="/usr/bin/gatttool -b " + this.address + " --char-read -a " + handle; 
    ...
    exec(gatt_cmd, function (error, stdout, stderr) {
        var hex;
        // Error detection
        if (error) {
            self.log(stderr);
...
PlaybulbCandle.prototype.writeHandle(handle,buf, callback){
    //todo: execute GATT command
    if (callback!=null)
        callback(null);
}
