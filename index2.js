...
PlaybulbPlatform.prototype.configureAccessory = function(homebridgeAcc) {
    ...
    if (this.configuredAccessories[homebridgeAcc.context['address']]==null){
        this.log.info("configureAccessory: %s not defined in config, removing", homebridgeAcc.context['address']);
        this.api.unregisterPlatformAccessories("homebridge-playbulb", "Playbulb", [homebridgeAcc]);
    }else{
        this.configuredAccessories[homebridgeAcc.context['address']].homebridgeAcc=homebridgeAcc;
    }
...
PlaybulbPlatform.prototype.bulbDiscovered = function(bulb) {
    ...
    if(address in this.candleAccessories){
        ...
        var c=this.candleAccessories[address]=bulb;
...
PlaybulbPlatform.prototype.connectCandle = function(error, bulb) {
    ...
    var homebridgeAcc = this.configuredAccessories[address].homebridgeAcc;

    if (homebridgeAcc==null){
        homebridgeAcc= new Accessory(candle.config.name, UUIDGen.generate(address));
...
PlaybulbPlatform.prototype.disconnectCandle = function(bulb, homebridgeAcc, error) {
    ...
    //this.api.unregisterPlatformAccessories("homebridge-playbulb", "Playbulb", [homebridgeAcc]);
    //delete this.configuredAccessories[address].homebridgeAcc;
    //this.configuredAccessories[address].homebridgeAcc=null;
